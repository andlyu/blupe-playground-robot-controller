...
While parked with auto-queue enabled, the UI shows **Stopped but ready** in green.
The controller reports `STOPPED_READY` while keeping session admission disabled
until the arms have moved Home. Pausing the queue returns the label to **Stopped**.
