...
3. **Poses and queue:** Use a `settings.poses_file` with ten arm joint angles and two gripper values, bound to the robot and calibration identity. Auto-queue is off at startup. Enable auto-queue in the Operator panel from enabled Home or saved Zero (including torque-off Zero). At Zero, the arms wait for work, then enable and move Home before acceptance. Visitor Stop, time expiry, and completion preserve auto-queue: return Home for waiting work, otherwise move to saved Zero and verify torque off. Later queued work enables both arms and returns Home before acceptance. Operator Hold/Pause, disconnect, and faults cancel auto-queue.
