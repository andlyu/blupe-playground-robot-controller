...
const aq=el('auto-queue');aq.hidden=!Array.isArray(s.gripper);aq.dataset.enabled=String(!!cloud.auto_queue);aq.textContent=cloud.auto_queue?'Disable Auto-queue':'Enable Auto-queue';aq.disabled=!cloud.auto_queue&&(!active||busy||!cloud.connected||!s.at_home);
