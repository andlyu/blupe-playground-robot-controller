...
<div id="queue-note" class="queue-note">Enable auto-queue at Home or saved Zero. From Zero, queued work enables the arms and moves Home before starting. Stop &amp; Hold or Pause Queue disables auto-queue.</div></div><div id="joints"''',page,count=1)
...
const aq=el('auto-queue');aq.hidden=false;aq.dataset.enabled=String(!!cloud.auto_queue);aq.textContent=cloud.auto_queue?'Disable Auto-queue':'Enable Auto-queue';aq.disabled=!cloud.auto_queue&&(busy||cloud.returning_home||!cloud.connected||s.calibration_ready===false||!!s.error||!s.home_captured||!s.zero_captured||!(active&&s.at_home||['active','readonly'].includes(s.mode)&&s.at_zero));
...
async function queueTick(){try{const r=await fetch('api/queue');if(!r.ok)throw Error('Queue unavailable');const q=await r.json();let entries=q.entries||[];if(q.current_session_id&&!entries.some(e=>e.session_id===q.current_session_id))entries=[{session_id:q.current_session_id,status:'active'},...entries];el('queue-count').textContent=entries.length;el('queue-list').innerHTML=entries.length?entries.map((e,i)=>`<li class="queue-item"><b>${i+1}</b><span class="queue-id">${esc(e.session_id)}</span><span class="queue-state">${esc(e.status||e.state||'queued')}</span></li>`).join(''):'<li class="queue-empty">Queue empty</li>';el('queue-note').textContent=q.message||'Enable auto-queue at Home or saved Zero. From Zero, queued work enables the arms and moves Home before starting. Stop & Hold or Pause Queue disables auto-queue.';}catch(e){el('queue-note').textContent=e.message}setTimeout(queueTick,2000)}tick();queueTick();
</script>'''
