...
<button id="zero" onclick="act('zero')">Move Zero</button><button id="run" class="run" onclick="act('cloud_ready')">Run via Session API</button>
    ...
    page = re.sub(r'<div class="queue-actions">.*?<div id="joints"', '''<div class="queue-actions"><button onclick="act('cloud_pause')">Pause Queue</button></div>
<div id="queue-note" class="queue-note">Each task requires explicit operator handoff.</div></div><div id="joints"''',page,count=1)
