...
def standard_page():
    ...
    controls = '''<div class="controls"><button class="stop" onclick="act('hold')">Stop &amp; Hold</button>
...
<button id="zero" onclick="act('zero')">Move Zero</button><button id="run" class="run" onclick="act('cloud_ready')">Run via Session API</button>
    ...
    page = re.sub(r'<div class="queue-actions">.*?<div id="joints"', '''<div class="queue-actions"><button id="autoqueue" onclick="toggleAutoqueue()">Autoqueue: Off</button><button onclick="act('cloud_pause')">Pause Queue</button></div>
<div id="queue-note" class="queue-note">Autoqueue returns home after successful tasks and accepts the next. Stop or errors pause the queue.</div></div><div id="joints"''',page,count=1)
...
const log=document.getElementById('log');let prior='',cameraRoles=[],autoqueue=false;
...
function toggleAutoqueue(){act(autoqueue?'cloud_pause':'cloud_autoqueue')}
...
async function tick(){try{const r=await fetch('api/status');if(!r.ok)throw Error('Controller unavailable');const s=await r.json();const cloud=typeof s.cloud_execution==='object'?s.cloud_execution:{};const active=s.mode==='active';const busy=cloud.session_active||cloud.queue_ready||cloud.auto_queue||s.manual_motion;autoqueue=cloud.auto_queue===true;
...
flag('api',cloud.connected,cloud.session_active?'CONNECTED / ACTIVE':'CONNECTED / IDLE','DISCONNECTED');el('source').textContent=(cloud.session_active||cloud.queue_ready)?'SESSION API':'OPERATOR';el('phase').textContent=cloud.returning_home?'RETURNING HOME':s.manual_motion?'MOVING TO POSE':s.manual_motion_error?s.manual_motion_error:cloud.command_active?'EXECUTING':cloud.queue_ready?'WAITING FOR TASK':'PAUSED';
...
el('autoqueue').textContent='Autoqueue: '+(autoqueue?'On':'Off');el('autoqueue').className=autoqueue?'run':'';el('autoqueue').setAttribute('aria-pressed',String(autoqueue));el('autoqueue').disabled=!autoqueue&&(!active||busy||!cloud.connected||s.at_home!==true);
el('joints').innerHTML=s.joints_deg.map((v,i)=>`<div class="joint"><b>${esc(s.joint_names[i])}</b><div class="track"><div class="fill" style="width:${Math.max(0,Math.min(100,(v+180)/360*100))}%"></div></div><span>${v.toFixed(1)}°</span></div>`).join('')+`<div class="joint"><b>gripper</b><div class="track"><div class="fill" style="width:${s.gripper*100}%"></div></div><span>${(s.gripper*100).toFixed(1)}%</span></div>`;
...
async function queueTick(){try{const r=await fetch('api/queue');if(!r.ok)throw Error('Queue unavailable');const q=await r.json();let entries=q.entries||[];if(q.current_session_id&&!entries.some(e=>e.session_id===q.current_session_id))entries=[{session_id:q.current_session_id,status:'active'},...entries];el('queue-count').textContent=entries.length;el('queue-list').innerHTML=entries.length?entries.map((e,i)=>`<li class="queue-item"><b>${i+1}</b><span class="queue-id">${esc(e.session_id)}</span><span class="queue-state">${esc(e.status||e.state||'queued')}</span></li>`).join(''):'<li class="queue-empty">Queue empty</li>';el('queue-note').textContent='Autoqueue returns home after successful tasks and accepts the next. Stop or errors pause the queue.';}catch(e){el('queue-note').textContent=e.message}setTimeout(queueTick,2000)}tick();queueTick();
</script>'''
